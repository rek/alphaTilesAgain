...
- [x] 1.2 Restart Metro and verify game shell header shows `"Score"` not `"Score: {{points}}"`
...
- [x] 2.3 Open game menu in running app and verify label shows `"Score: <N>"` with real number
...
- [x] 3.4 Test on web: boot app, tap "Tap to begin", confirm audio plays in game

## 4. Player Deletion Smoke Test

- [x] 4.1 Navigate to player management, delete a player, confirm it is removed from the list
- [x] 4.2 Confirm app does not crash and navigates correctly after deletion

## 5. Validation

- [x] 5.1 Run `npx tsc --noEmit` — zero errors
- [x] 5.2 Run `openspec validate qa-remaining-flows` — exits 0
- [x] 5.3 Manual smoke: game header shows `"Score"` label + separate number
- [x] 5.4 Manual smoke: game menu shows `"Score: N"` with live value
- [x] 5.5 Manual smoke: web audio plays after tap-to-begin
