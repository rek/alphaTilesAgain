...
The prebuild pipeline SHALL execute in the order: rsync → validate → generate-manifest → expo-config-resolve. Each stage MUST depend on its predecessor via NX `dependsOn` declarations.
...
#### Scenario: Validation failure aborts the pipeline

- **WHEN** the rsync succeeds but the pack fails structural validation
- **THEN** the generate-manifest stage DOES NOT run and the build exits non-zero with the validator's error report
