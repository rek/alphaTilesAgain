...
- **Storybook host needs explicit lib registration.** Adding a `*.stories.tsx` is not enough — also append the lib to `libs/shared/storybook-host/.storybook/main.ts` `stories` array (one entry per lib `src` directory). Stories in unregistered libs are silently invisible. Many `feature-game-*` libs are currently unregistered.
